 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const cE="storage";function yy(t,e,n){return t=_e(t),DP(t,e,n)}function vy(t){return t=_e(t),AP(t)}function wy(t){return t=_e(t),OP(t)}function ka(t,e){return t=_e(t),$P(t,e)}function Ia(t=_f(),e){t=_e(t);const r=ou(t,cE).getImmediate({identifier:e}),i=J0("storage");return i&&UP(r,...i),r}function UP(t,e,n,r={}){MP(t,e,n,r)}function FP(t,{instanceIdentifier:e}){const n=t.getProvider("app").getImmediate(),r=t.getProvider("auth-internal"),i=t.getProvider("app-check-internal");return new Pp(n,r,i,e,Gi)}function VP(){zr(new ar(cE,FP,"PUBLIC").setMultipleInstances(!0)),sn(my,gy,""),sn(my,gy,"esm2017")}VP();async function BP(t){const e=Ue().currentUser.uid,n=Is(it(),`users/${e}/habits/${t}/todos`),r=await ED(n),i=[];return r.forEach(s=>i.push(s.data())),i.sort((s,o)=>s.index-o.index),i}function zP(){const[t,e]=A.exports.useState(!1),[n,r]=A.exports.useState([]),[i,s]=A.exports.useState(!1);async function o(){try{const _=new pn;await FA(Ue(),_)}catch(_){console.log(`ERROR, COULDN'T SIGN IN: ${_}`),s(!0)}}async function a(){await mA(Ue())}async function l(_,k,S){try{const I=Ue().currentUser.uid,P=await Uc(Is(it(),`users/${I}/habits`),_),O=Is(P,"todos"),F=`users/${I}/${k.name}`,z=ka(Ia(),F),L=await yy(z,k),U=await vy(z),T=L.metadata.fullPath;for(let b=0;b<S.length;b++){const K=await Uc(O,S[b]);await At(K,{refId:K.id})}await At(P,{image:U,storageURI:T,refId:P.id})}catch(I){console.log(`ERROR CREATING HABIT: ${I}`),s(!0)}}async function u(_,k){try{const S=Ue().currentUser.uid,I=ka(Ia(),_.storageURI),P=Ot(it(),`users/${S}/habits`,_.refId),O=`users/${S}/${_.refId}/${k.name}`,F=ka(Ia(),O);let z,L,U;At(P,{image:null}),z=await yy(F,k),L=await vy(F),U=z.metadata.fullPath,await At(P,{image:L,storageURI:U}),await wy(I)}catch(S){console.log(`ERROR WHILE ATTEMPMTING TO REPLACE IMAGE: ${S}`),s(!0)}}async function c(_,k){try{const S=Ue().currentUser.uid,I=Ot(it(),`users/${S}/habits/${_}`);await At(I,{[k.label]:k.value})}catch(S){console.log(`ERROR UPDATING DETAIL: ${S}`),s(!0)}}function h(_,k,S,I){const P=n.findIndex(b=>b.refId===k),O=n[P],F=O.todos.findIndex(b=>b.refId===S),z=O.todos[F];let L,U,T;return L=[...z.dates],L.includes(I)?L=L.filter(b=>b!==I):L.push(I),U={...z,dates:L},T=O.todos.slice(0,F).concat(U).concat(O.todos.slice(F+1)),_.slice(0,P).concat({...O,todos:T}).concat(_.slice(P+1))}async function d(_,k,S){try{const I=Ue().currentUser.uid,P=Ot(it(),`users/${I}/habits/${_}/todos/${k}`),O=await _D(P);let F;r(z=>h(z,_,k,S)),F=O.data().dates,F.includes(S)?F=F.filter(z=>z!==S):F.push(S),await At(P,{dates:F})}catch(I){console.log(I),s(!0)}}async function p(_){try{const k=Ue().currentUser.uid,S=Ot(it(),`users/${k}/habits/${_}`),I=n.find(F=>F.refId===_),P=I.todos.map(F=>F.refId),O=ka(Ia(),I.storageURI);await wy(O);for(let F of P){const z=Ot(it(),`user/${k}/habits/${_}/todos/${F}`);await Mc(z)}await Mc(S),E()}catch(k){console.log(`ERROR, COULDN'T DELETE HABIT : ${k}`),s(!0)}}async function y(_,k,S,I){try{const P=Ue().currentUser.uid,O=Ot(it(),`users/${P}/habits/${_}/todos/${k}`),F=n.findIndex(N=>N.refId===_),z=n[F],L=z.todos.findIndex(N=>N.refId===k),U=z.todos[L],T=U.dates.filter(N=>bi(new Date(N),S)),b=z.todos.slice(0,L).concat({...U,range:S,dates:T}).concat(z.todos.slice(L+1));let K;if(K=n.slice(0,F).concat({...z,todos:b}).concat(n.slice(F+1)),await At(O,{range:S,dates:T}),r(K),I){const N=w(K,z.refId),B=Ot(it(),`users/${P}/habits/${_}/todos/${z.todos[0].refId}`),q={name:z.name,id:Si(),range:{from:"free",to:"free"},dates:[...z.todos[0].dates],index:z.todos.length};K=N.data,await At(B,{dates:[]}),C(_,q)}}catch(P){console.log(`ERORR, COULDN'T TOGGLE RANGE: ${P}`),s(!0)}}function w(_,k){const S=_.findIndex(F=>F.refId===k),I=_[S],P={name:I.name,id:Si(),range:{from:"free",to:"free"},dates:[...I.todos[0].dates]},O=[...I.todos.slice(1),P];return O.unshift({...I.todos[0],dates:[]}),{data:_.slice(0,S).concat({...I,todos:O}).concat(_.slice(S+1)),todo:P}}async function C(_,k){try{const S=n.findIndex(U=>U.refId===_),I=n[S],P=I.todos[0].dates.filter(U=>U!=k.range.from),O=[{...I.todos[0],dates:P},...I.todos.slice(1),k],F=Ue().currentUser.uid,z=Is(it(),`users/${F}/habits/${_}/todos`),L=await Uc(z,k);if(k.refId=L.id,r(U=>U.slice(0,S).concat({...I,todos:O}).concat(U.slice(S+1))),I.todos[0].dates.some(U=>U===k.range.from)){const U=Ot(it(),`users/${F}/habits/${_}/todos/${I.todos[0].refId}`),T=I.todos[0].dates.filter(b=>b!==k.range.from);await At(U,{dates:T})}await At(L,{refId:L.id})}catch(S){console.log(`ERORR, COULDN'T ADD TODO: ${S}`),s(!0)}}async function m(_,k){try{const S=Ue().currentUser.uid,I=Ot(it(),`users/${S}/habits/${_}/todos/${k}`),P=n.findIndex(z=>z.refId===_),O=n[P],F=O.todos.filter(z=>z.refId!==k);F.forEach(async(z,L)=>{if(!L)return;const U=Ot(it(),`users/${S}/habits/${_}/todos/${z.refId}`);await At(U,{index:L})}),r(z=>z.slice(0,P).concat({...O,todos:F}).concat(z.slice(P+1))),await Mc(I)}catch(S){console.log(`ERROR DELETING TODO: ${S}`),s(!0)}}async function f(_,k,S){try{const I=Ue().currentUser.uid,P=Ot(it(),`users/${I}/habits/${_}/todos/${k}`),O=n.findIndex(T=>T.refId===_),F=n[O],z=F.todos.findIndex(T=>T.refId===k),L=F.todos[z],U=F.todos.slice(0,z).concat({...L,name:S}).concat(F.todos.slice(z+1));r(T=>T.slice(0,O).concat({...F,todos:U}).concat(T.slice(O+1))),await At(P,{name:S})}catch(I){console.log(`ERROR UPDATING TODO NAME: ${I}`),s(!0)}}function g(){const _=Ue().currentUser.uid,k=fD(Is(it(),`users/${_}/habits`),mD("timestamp","asc"));return SD(k,I=>I.docChanges().forEach(async P=>{if(P.type==="removed")r(O=>O.filter(F=>F.id!==P.doc.id));else{const O=P.doc.data(),F=await BP(O.refId);r(z=>{const L=z.findIndex(U=>U.id===O.id);return~L?z.slice(0,L).concat({...O,todos:F}).concat(z.slice(L+1)):[...z,O]})}}))}function E(){window.location.href="/Habit-Tracker"}return A.exports.useEffect(()=>{rw(xD)},[]),A.exports.useEffect(()=>{let _;async function k(){console.log("having fun"),await a(),await o()}return Ue().currentUser&&k(),pA(Ue(),()=>{e(()=>!!Ue().currentUser),Ue().currentUser&&(_=g())}),()=>_?_():null},[]),v("div",{className:"App",children:M(bI,{children:[v(gl,{content:M("div",{className:"state-of-horror",children:[v("span",{className:"horror"}),v("p",{children:"Error processing your request, please connect to the internet and try again!"}),v("button",{className:"go-home",onClick:E})]}),close:E,shown:i}),M("nav",{className:"navbar",children:[v("div",{className:"logo",children:"Kerosene"}),M("ul",{className:"links",children:[v("li",{children:v(Kg,{path:"/Habit-Tracker",icon:ND})}),v("li",{children:v(Kg,{path:"/Habit-Tracker/info",icon:RD})}),t?v("li",{children:v("button",{className:"log-out-button",onClick:a})}):null]})]}),t?M(DI,{children:[v(_s,{element:v(CD,{habits:n,update:d,add_habit:l}),path:"/Habit-Tracker"}),v(_s,{element:v(MI,{}),path:"/Habit-Tracker/info"}),n.map(_=>v(_s,{path:`/Habit-Tracker/habits/${_.id}`,element:v(HI,{habit:_,update_cell:d,update_detail:c,change_image:u,delete_habit:p,toggle_habit_range:y,add_todo_to_habit:C,delete_todo:m,rename_todo:f})},_.id)),v(_s,{path:"*",element:v("p",{children:"Not found"})})]}):v("div",{className:"sign-in-content",children:v("button",{onClick:o,children:"Log In"})}),M("footer",{className:"footer",children:["A project by"," ",v("a",{href:"https://ol.reddit.com/u/_by_me",target:"_blank",children:"u/_by_me"}),", \xA9 2022"]})]})})}qc.createRoot(document.getElementById("root")).render(v(Ny.StrictMode,{children:v(zP,{})}));
